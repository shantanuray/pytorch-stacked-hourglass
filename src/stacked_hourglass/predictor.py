...
class GenericPosePredictor:
    """Predictor for generic data set."""
    def __init__(self, model, device=None):
        if device is None:
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
        device = torch.device(device)
    ...
    def prepare_image(self, image, mean, stddev):
        was_fixed_point = not image.is_floating_point()
        image = torch.empty(image.shape, device='cpu', dtype=torch.float32).copy_(image)
        if was_fixed_point:
            image /= 255.0
    ...
    def estimate_joints(self, images, flip=False):
        ...
        Args:
            images: The images to estimate joint locations for.
                    Can be a single image or a list of images.
            flip (bool): If set to true, evaluates on flipped versions
                    of the images as well and averages the results.

        Returns:
            The predicted joint locations.
        """
        is_batched = _check_batched(images)
        raw_images = images if is_batched else images.unsqueeze(0)
        heatmaps = self.estimate_heatmaps(raw_images, flip=flip).cpu()
